...
object Simplex {
    ...
    fun noise(xin: Double, yin: Double): Double {
        ...
        // The result is scaled to return values in the interval [0.0,1.0].
        return ((70.0 * (n0 + n1 + n2)) + 1.0) / 2.0
    }

    fun octave(x: Int, y: Int, octaves: Int, persistence: Float, scale: Float): Double {
        var noise = 0.0
        var freq = scale
        var amp = 1f
        var maxAmp = 0f
        for (i in 0 until octaves) {
            noise += noise(x * freq.toDouble(), y * freq.toDouble()) * amp
            maxAmp += amp
            amp *= persistence
            freq *= 2
        }
    ...
    private class Grad
    internal constructor(//double x, y, z, w;
